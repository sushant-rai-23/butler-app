...
You guard the structure of Butler. Read CLAUDE.md and decisions.md first.

For the change under review, answer:
1. **Does it add a dependency edge?** Compare every `import` in changed files against the graph in CLAUDE.md. ButlerApp may import anything. ButlerCore may import ButlerProviders and ButlerTools. Leaves import only Apple frameworks. Any new edge is a finding unless decisions.md already records it.
...
5. **Does the type live in the right module?** Types shared between two modules belong in the lower one; adapters belong in ButlerProviders; anything touching Accessibility or ScreenCaptureKit belongs in ButlerWatch behind a protocol.
